...
func (h *SettingHandler) GetPaymentSettings(c *gin.Context) {
	ps, err := h.settingService.GetPaymentSettings(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	response.Success(c, dto.PaymentSettings{
		PaymentEnabled:             ps.PaymentEnabled,
		PaymentCurrency:            ps.PaymentCurrency,
		PaymentExchangeRate:        ps.PaymentExchangeRate,
		PaymentPresetAmounts:       ps.PaymentPresetAmounts,
		PaymentMinAmount:           ps.PaymentMinAmount,
		PaymentMaxAmount:           ps.PaymentMaxAmount,
		AlipayEnabled:              ps.AlipayEnabled,
		AlipayAppID:                ps.AlipayAppID,
		AlipayPrivateKeyConfigured: ps.AlipayPrivateKeyConfigured,
		AlipayPublicKeyConfigured:  ps.AlipayPublicKeyConfigured,
		AlipayF2FEnabled:           ps.AlipayF2FEnabled,
		WechatEnabled:              ps.WechatEnabled,
		WechatAppID:                ps.WechatAppID,
		WechatMchID:                ps.WechatMchID,
		WechatAPIKeyConfigured:     ps.WechatAPIKeyConfigured,
		EpayEnabled:                ps.EpayEnabled,
		EpayAPIURL:                 ps.EpayAPIURL,
		EpayPID:                    ps.EpayPID,
		EpayKeyConfigured:          ps.EpayKeyConfigured,
		EpayType:                   ps.EpayType,
		ReferralEnabled:            ps.ReferralEnabled,
		ReferralCommissionRate:     ps.ReferralCommissionRate,
	})
}

// UpdatePaymentSettings 更新支付设置
// PUT /api/v1/admin/settings/payment
func (h *SettingHandler) UpdatePaymentSettings(c *gin.Context) {
	var req dto.UpdatePaymentSettingsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	...
	if err := h.settingService.UpdatePaymentSettings(c.Request.Context(), ps); err != nil {
		response.ErrorFrom(c, err)
		return
	}

	// 重新获取更新后的设置返回
	updated, err := h.settingService.GetPaymentSettings(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	response.Success(c, dto.PaymentSettings{
		...
		PaymentExchangeRate:        updated.PaymentExchangeRate,
		PaymentPresetAmounts:       updated.PaymentPresetAmounts,
		PaymentMinAmount:           updated.PaymentMinAmount,
		PaymentMaxAmount:           updated.PaymentMaxAmount,
		AlipayEnabled:              updated.AlipayEnabled,
		AlipayAppID:                updated.AlipayAppID,
		AlipayPrivateKeyConfigured: updated.AlipayPrivateKeyConfigured,
		AlipayPublicKeyConfigured:  updated.AlipayPublicKeyConfigured,
		AlipayF2FEnabled:           updated.AlipayF2FEnabled,
		WechatEnabled:              updated.WechatEnabled,
		WechatAppID:                updated.WechatAppID,
		WechatMchID:                updated.WechatMchID,
		WechatAPIKeyConfigured:     updated.WechatAPIKeyConfigured,
		EpayEnabled:                updated.EpayEnabled,
		EpayAPIURL:                 updated.EpayAPIURL,
		EpayPID:                    updated.EpayPID,
		EpayKeyConfigured:          updated.EpayKeyConfigured,
		EpayType:                   updated.EpayType,
