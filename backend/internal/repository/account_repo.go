...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, ungrouped bool) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if search != "" {
		q = q.Where(dbpredicate.Account(func(s *entsql.Selector) {
			nameCol := s.C(dbaccount.FieldName)
			credCol := s.C(dbaccount.FieldCredentials)
			pattern := "%" + search + "%"
			s.Where(entsql.Or(
				entsql.Like(nameCol, pattern),
				entsql.P(func(b *entsql.Builder) {
					b.WriteString(credCol)
					b.WriteString("->>'base_url' ILIKE ")
					b.Arg(pattern)
				}),
			))
		}))
