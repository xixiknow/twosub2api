...
func (s *SettingService) UpdateSettings(ctx context.Context, settings *SystemSettings) error {
	...
	// 登录 IP 提醒
	updates[SettingKeyLoginIPAlertEnabled] = strconv.FormatBool(settings.LoginIPAlertEnabled)
...
func (s *SettingService) GetPaymentSettings(ctx context.Context) (*PaymentSettings, error) {
	keys := []string{
		...
		SettingKeyWechatMchID,
		SettingKeyWechatAPIKey,
		SettingKeyEpayEnabled,
		SettingKeyEpayAPIURL,
		SettingKeyEpayPID,
		SettingKeyEpayKey,
		SettingKeyEpayType,
		SettingKeyReferralEnabled,
		SettingKeyReferralCommissionRate,
	}
	...
	result := &PaymentSettings{
		...
		PaymentCurrency:            s.getStringOrDefault(settings, SettingKeyPaymentCurrency, "CNY"),
		PaymentPresetAmounts:       s.getStringOrDefault(settings, SettingKeyPaymentPresetAmounts, "10,20,50,100"),
		AlipayEnabled:              settings[SettingKeyAlipayEnabled] == "true",
		AlipayAppID:                settings[SettingKeyAlipayAppID],
		AlipayPrivateKeyConfigured: settings[SettingKeyAlipayPrivateKey] != "",
		AlipayPublicKeyConfigured:  settings[SettingKeyAlipayPublicKey] != "",
		AlipayF2FEnabled:           settings[SettingKeyAlipayF2FEnabled] == "true",
		WechatEnabled:              settings[SettingKeyWechatEnabled] == "true",
		WechatAppID:                settings[SettingKeyWechatAppID],
		WechatMchID:                settings[SettingKeyWechatMchID],
		WechatAPIKeyConfigured:     settings[SettingKeyWechatAPIKey] != "",
		...
		EpayKeyConfigured:          settings[SettingKeyEpayKey] != "",
		EpayType:                   s.getStringOrDefault(settings, SettingKeyEpayType, "alipay"),
		ReferralEnabled:            settings[SettingKeyReferralEnabled] == "true",
	}

	// 解析浮点数
	if rate, err := strconv.ParseFloat(settings[SettingKeyPaymentExchangeRate], 64); err == nil {
		result.PaymentExchangeRate = rate
	} else {
		result.PaymentExchangeRate = 1
	}
	if min, err := strconv.ParseFloat(settings[SettingKeyPaymentMinAmount], 64); err == nil {
		result.PaymentMinAmount = min
	} else {
		result.PaymentMinAmount = 1
	}
	if max, err := strconv.ParseFloat(settings[SettingKeyPaymentMaxAmount], 64); err == nil {
		result.PaymentMaxAmount = max
	} else {
		result.PaymentMaxAmount = 10000
	}
...
func (s *SettingService) UpdatePaymentSettings(ctx context.Context, ps *PaymentSettings) error {
	updates := map[string]string{
		SettingKeyPaymentEnabled:         strconv.FormatBool(ps.PaymentEnabled),
		SettingKeyPaymentCurrency:        ps.PaymentCurrency,
		SettingKeyPaymentExchangeRate:    strconv.FormatFloat(ps.PaymentExchangeRate, 'f', 8, 64),
		SettingKeyPaymentPresetAmounts:   ps.PaymentPresetAmounts,
		SettingKeyPaymentMinAmount:       strconv.FormatFloat(ps.PaymentMinAmount, 'f', 2, 64),
		SettingKeyPaymentMaxAmount:       strconv.FormatFloat(ps.PaymentMaxAmount, 'f', 2, 64),
		SettingKeyAlipayEnabled:          strconv.FormatBool(ps.AlipayEnabled),
		SettingKeyAlipayAppID:            ps.AlipayAppID,
		SettingKeyAlipayF2FEnabled:       strconv.FormatBool(ps.AlipayF2FEnabled),
		SettingKeyWechatEnabled:          strconv.FormatBool(ps.WechatEnabled),
		SettingKeyWechatAppID:            ps.WechatAppID,
		SettingKeyWechatMchID:            ps.WechatMchID,
		SettingKeyEpayEnabled:            strconv.FormatBool(ps.EpayEnabled),
		SettingKeyEpayAPIURL:             ps.EpayAPIURL,
		SettingKeyEpayPID:                ps.EpayPID,
		SettingKeyEpayType:               ps.EpayType,
		SettingKeyReferralEnabled:        strconv.FormatBool(ps.ReferralEnabled),
		SettingKeyReferralCommissionRate: strconv.FormatFloat(ps.ReferralCommissionRate, 'f', 8, 64),
	}

	// 敏感字段：只有非空时才更新（保留已有值）
	if ps.AlipayPrivateKey != "" {
		updates[SettingKeyAlipayPrivateKey] = ps.AlipayPrivateKey
	}
	if ps.AlipayPublicKey != "" {
		updates[SettingKeyAlipayPublicKey] = ps.AlipayPublicKey
	}
	if ps.WechatAPIKey != "" {
		updates[SettingKeyWechatAPIKey] = ps.WechatAPIKey
	}
	if ps.EpayKey != "" {
		updates[SettingKeyEpayKey] = ps.EpayKey
	}
...
// IsBackendModeEnabled 检查是否启用后端模式
func (s *SettingService) IsBackendModeEnabled(ctx context.Context) bool {
	value, err := s.settingRepo.GetValue(ctx, "backend_mode_enabled")
	if err != nil {
		return false
	}
	return value == "true"
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	// 登录 IP 提醒
	result.LoginIPAlertEnabled = settings[SettingKeyLoginIPAlertEnabled] == "true"
...
// IsLoginIPAlertEnabled 检查是否启用登录 IP 异常提醒
func (s *SettingService) IsLoginIPAlertEnabled(ctx context.Context) bool {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyLoginIPAlertEnabled)
	if err != nil {
		return false
	}
	return value == "true"
...
// GetReferralCommissionRate 获取推荐返利比例 (0~1)
