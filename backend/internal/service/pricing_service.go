...
// ModelPricingDisplayItem 用于前端展示的模型定价项
...
// GetAllModelPricing 返回所有模型定价数据及最后更新时间
func (s *PricingService) GetAllModelPricing() ([]ModelPricingDisplayItem, time.Time) {
	...
	for name, p := range s.pricingData {
		...
		if p.SupportsPromptCaching {
			cacheRead := p.CacheReadInputTokenCost * 1_000_000
			cacheCreate := p.CacheCreationInputTokenCost * 1_000_000
