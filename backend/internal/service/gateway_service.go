...
const (
	...
	postUsageBillingTimeout    = 30 * time.Second // 事后扣费超时
...
// ActualNewInputTokens 计算实际的新输入 token 数量（排除缓存读取 token）。
// 原版 Anthropic API 的 input_tokens 包含 cache_read_input_tokens，需要减去；
// 但部分上游（如 New API / One API）返回的 input_tokens 已经排除了缓存 token，
// 此时 InputTokens < CacheReadInputTokens，减去后为负数，应直接使用 InputTokens。
func (u *ClaudeUsage) ActualNewInputTokens() int {
	if u.CacheReadInputTokens <= 0 {
		return u.InputTokens
	}
	actual := u.InputTokens - u.CacheReadInputTokens
	if actual < 0 {
		// 上游已将缓存 token 从 input_tokens 中分离，直接使用原值
		return u.InputTokens
	}
	return actual
}
...
type ForwardResult struct {
	...
	// ReasoningEffort records the reasoning effort level for Claude requests.
...
func (s *GatewayService) isModelSupportedByAccount(account *Account, requestedModel string) bool {
	...
	// 模型支持检查使用原始模型名（短 ID），与用户配置的 model_mapping 一致。
	// NormalizeModelID 仅在构建上游请求时使用，不应影响支持检查。
...
// removeEmptyTextBlocks 移除 messages 和 system 中的空 text block
// 空 text block（如 {"type":"text","text":""}）会导致 Claude API 返回 400 错误
func removeEmptyTextBlocks(body []byte) []byte {
	var data map[string]any
	if err := json.Unmarshal(body, &data); err != nil {
		return body
	}
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	// 清理空 text block，避免上游返回 400 错误
	body = removeEmptyTextBlocks(body)
	...
	if err != nil {
		logger.LegacyPrintf("service.gateway", "[Forward] GetAccessToken failed: account=%s err=%v", account.Name, err)
	...
	// 调试日志：记录即将转发的账号信息
	logger.LegacyPrintf("service.gateway", "[Forward] Using account: ID=%d Name=%s Platform=%s Type=%s TLSFingerprint=%v Proxy=%s BaseURL=%s",
		account.ID, account.Name, account.Platform, account.Type, account.IsTLSFingerprintEnabled(), proxyURL, account.GetBaseURL())
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if err != nil {
			logger.LegacyPrintf("service.gateway", "[Forward] buildUpstreamRequest failed: account=%s(%d) base_url=%s err=%v", account.Name, account.ID, account.GetBaseURL(), err)
			return nil, &UpstreamFailoverError{StatusCode: 502}
		}
		logger.LegacyPrintf("service.gateway", "[Forward] Upstream URL: %s | Account: %s(%d) Type=%s Platform=%s", upstreamReq.URL.String(), account.Name, account.ID, account.Type, account.Platform)
		...
		if err != nil {
			...
			// 返回 UpstreamFailoverError 让 handler 层 failover 循环切换到下一个账号重试
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthrough(
	...
	token, tokenType, err := s.GetAccessToken(ctx, account)
	if err != nil {
		logger.LegacyPrintf("service.gateway", "[Passthrough] GetAccessToken failed: account=%s err=%v", account.Name, err)
		return nil, &UpstreamFailoverError{StatusCode: 502}
	}
	if tokenType != "apikey" {
		return nil, &UpstreamFailoverError{StatusCode: 502}
...
func (s *GatewayService) isThinkingBlockSignatureError(respBody []byte) bool {
	...
	// 检测 thinking block 签名相关的错误（精确匹配）
	// 例如: "Invalid `signature` in `thinking` block", "thought_signature is required", "Invalid thought_signature"
	if strings.Contains(msg, "thought_signature") ||
		(strings.Contains(msg, "signature") && (strings.Contains(msg, "thinking") || strings.Contains(msg, "redacted_thinking"))) {
		logger.LegacyPrintf("service.gateway", "[SignatureCheck] Detected thinking block signature error")
		return true
	}
...
func (s *GatewayService) handleErrorResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account) (*ForwardResult, error) {
	...
	// 400 是客户端请求错误，直接返回上游响应体，不触发 failover
	if resp.StatusCode == 400 {
...
type RecordUsageInput struct {
	...
	InboundEndpoint    string             // 入站端点路径
	UpstreamEndpoint   string             // 上游端点路径
	UpstreamModel      string             // 上游实际模型（模型映射时与请求模型不同）
}
...
// detachedBillingContext 创建一个不会被上游取消的计费上下文
...
// resolveUsageBillingRequestID 从 context / 上游响应 / 随机生成 中选出一个全局唯一的计费请求 ID
...
// resolveUsageBillingPayloadFingerprint 选出请求体指纹（优先 payload hash，其次 request ID）
...
// buildUsageBillingCommand 构造幂等扣费命令
...
// applyUsageBilling 执行幂等扣费（如果 repo 可用，则走原子去重路径；否则降级到老路径）
...
// finalizePostUsageBilling 在原子扣费成功后更新缓存计数（不再做 DB 写，只做 Redis 缓存同步）
...
// writeUsageLogBestEffort 以 best-effort 方式写入使用日志（不影响计费结果）
...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	...
	if cost != nil {
		// 已按次计费，跳过其他计费逻辑
	} else if result.MediaType == "image" || result.MediaType == "video" {
	...
	// 添加端点路径
	if input.InboundEndpoint != "" {
		usageLog.InboundEndpoint = &input.InboundEndpoint
	}
	if input.UpstreamEndpoint != "" {
		usageLog.UpstreamEndpoint = &input.UpstreamEndpoint
	}

	// 记录上游模型（仅当与请求模型不同时）
	usageLog.UpstreamModel = optionalNonEqualStringPtr(input.UpstreamModel, result.Model)
