...
// IsModelSupported 检查模型是否在 model_mapping 中（支持通配符 + 智能归一化）
// 如果未配置 mapping，返回 true（允许所有模型）
// 匹配优先级：精确匹配 > 通配符匹配 > 归一化后精确匹配 > 归一化后通配符匹配
func (a *Account) IsModelSupported(requestedModel string) bool {
	...
	// 归一化后匹配（处理 4.6 vs 4-6 等变体）
	normalizedReq := claude.NormalizeModelName(requestedModel)
	if normalizedReq != requestedModel {
		if _, exists := mapping[normalizedReq]; exists {
			return true
		}
		for pattern := range mapping {
			if matchWildcard(pattern, normalizedReq) {
				return true
			}
		}
	}
	// 对 mapping key 也做归一化再匹配（用户配置的 key 可能用了点号）
	for pattern := range mapping {
		normalizedPattern := claude.NormalizeModelName(pattern)
		if normalizedPattern != pattern {
			if normalizedPattern == requestedModel || normalizedPattern == normalizedReq {
				return true
			}
			if matchWildcard(normalizedPattern, requestedModel) || matchWildcard(normalizedPattern, normalizedReq) {
				return true
			}
		}
	}
...
// 支持智能归一化匹配：如果精确/通配符匹配失败，会尝试归一化后再匹配。
func (a *Account) ResolveMappedModel(requestedModel string) (mappedModel string, matched bool) {
	...
	// 通配符匹配（最长优先）
	if result, ok := matchWildcardMappingResult(mapping, requestedModel); ok {
		return result, true
	}
	// 归一化后匹配（处理 4.6 vs 4-6 等变体）
	normalizedReq := claude.NormalizeModelName(requestedModel)
	if normalizedReq != requestedModel {
		if mappedModel, exists := mapping[normalizedReq]; exists {
			return mappedModel, true
		}
		if result, ok := matchWildcardMappingResult(mapping, normalizedReq); ok {
			return result, true
		}
	}
	// 对 mapping key 也做归一化再匹配
...
// matchWildcardMappingResult 通配符映射匹配（最长优先），返回是否命中。
...
// IsOpenAICompatible returns true if the account belongs to an OpenAI-compatible platform
// (Qwen, DeepSeek, GLM, Kimi, iFlow).
func (a *Account) IsOpenAICompatible() bool {
	switch a.Platform {
	case PlatformQwen, PlatformDeepSeek, PlatformGLM, PlatformKimi, PlatformIFlow:
		return true
	default:
		return false
	}
...
// getExtraString 从 Extra 中读取指定 key 的 string 值
func (a *Account) getExtraString(key string) string {
	if a.Extra == nil {
		return ""
	}
