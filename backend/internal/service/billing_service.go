...
func (s *BillingService) GetModelPricing(model string) (*ModelPricing, error) {
	...
	// 3. 通用兜底：未知模型按中等价位（GPT-5.1 级别）计费，避免 ActualCost=0 导致免费使用
	log.Printf("[Billing] WARNING: No pricing found for model '%s', using universal fallback (gpt-5.1 level pricing). Please add pricing config for this model.", model)
	return s.applyModelSpecificPricingPolicy(model, &ModelPricing{
		InputPricePerToken:         1.25e-6, // $1.25 per MTok
		OutputPricePerToken:        10e-6,   // $10 per MTok
		CacheCreationPricePerToken: 1.25e-6, // $1.25 per MTok
