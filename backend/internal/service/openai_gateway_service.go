...
func resolveOpenAIWSFallbackErrorResponse(err error) (statusCode int, errType string, clientMessage string, upstreamMessage string, ok bool) {
	...
	case "invalid_encrypted_content":
		...
		errType = "invalid_request_error"
		if upstreamMessage == "" {
			upstreamMessage = "invalid encrypted content"
		}
...
// ResolveGroupByID resolves a group by its ID for fallback group lookup.
...
func (s *OpenAIGatewayService) GetAccessToken(ctx context.Context, account *Account) (string, string, error) {
	...
	case AccountTypeUpstream:
		...
		if apiKey == "" {
			return "", "", errors.New("upstream account missing api_key credential")
		}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	// For APIKey/Upstream accounts cached as not supporting /v1/responses,
	// directly use Chat Completions conversion path.
	if (account.Type == AccountTypeAPIKey || account.Type == AccountTypeUpstream) && isResponsesNotSupported(account.ID) {
		mappedModel := account.GetMappedModel(reqModel)
		return s.ForwardResponsesAsChatCompletions(ctx, c, account, body, reqModel, mappedModel, reqStream, startTime)
	}
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	case AccountTypeUpstream:
		...
		if baseURL == "" {
			return nil, errors.New("upstream account missing base_url credential")
		}
		validatedURL, err := s.validateUpstreamBaseURL(baseURL)
		if err != nil {
			return nil, err
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	if err := scanner.Err(); err != nil {
		if clientDisconnected {
			...
			return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: clientDisconnected}, nil
		}
		...
		if errors.Is(err, bufio.ErrTooLong) {
			...
			return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: clientDisconnected}, err
		}
		logger.LegacyPrintf("service.openai_gateway",
		...
		return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: clientDisconnected}, fmt.Errorf("stream read error: %w", err)
	}
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	case AccountTypeAPIKey:
		...
		baseURL := account.GetOpenAIBaseURL()
		if baseURL == "" {
			// 非 OpenAI 平台的 APIKey 账号（如被分配到 OpenAI 分组的第三方中转），
			// 尝试使用通用 base_url
			baseURL = strings.TrimSpace(account.GetCredential("base_url"))
		}
		if baseURL == "" {
	...
	case AccountTypeUpstream:
		// Upstream accounts use base_url from credentials
		baseURL := strings.TrimSpace(account.GetCredential("base_url"))
		if baseURL == "" {
			return nil, errors.New("upstream account missing base_url credential")
		}
...
type OpenAIRecordUsageInput struct {
	...
	RequestPayloadHash string // 请求体语义哈希
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// 记录上游模型（仅当与请求模型不同时）
	usageLog.UpstreamModel = optionalNonEqualStringPtr(billingModel, result.Model)
	if input.UserAgent != "" {
...
// codexUsageProgress holds utilization and reset time extracted from account extra.
type codexUsageProgress struct {
	Utilization float64
	ResetsAt    *time.Time
}

// buildCodexUsageProgressFromExtra extracts codex usage progress for a given window
// (e.g. "7d" or "5h") from the account's Extra map.
// It reads keys like "codex_7d_used_percent" and "codex_7d_reset_at".
func buildCodexUsageProgressFromExtra(extra map[string]any, window string, now time.Time) *codexUsageProgress {
	percentKey := "codex_" + window + "_used_percent"
	resetAtKey := "codex_" + window + "_reset_at"

	percentVal, ok := extra[percentKey]
	if !ok {
		return nil
	}
	var pct float64
	...
	progress := &codexUsageProgress{Utilization: pct}

	if resetStr, ok := extra[resetAtKey].(string); ok && resetStr != "" {
