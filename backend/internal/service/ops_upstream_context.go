...
// SetOpsUpstreamErrorFromFailover 在 failover 耗尽时记录真实上游状态码和错误信息。
// 应在状态码映射之前调用，确保运维日志中保留原始上游响应信息。
func SetOpsUpstreamErrorFromFailover(c *gin.Context, upstreamStatusCode int, responseBody []byte) {
	if c == nil {
		return
	}
	msg := ExtractUpstreamErrorMessage(responseBody)
	setOpsUpstreamError(c, upstreamStatusCode, msg, "")
}
