...
func TestSettingService_UpdateSettings_PersistsUserFeatureSwitches(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		ModelSquareEnabled:       false,
		AvailabilityCheckEnabled: false,
	})
	require.NoError(t, err)
	require.Equal(t, "false", repo.updates[SettingKeyModelSquareEnabled])
	require.Equal(t, "false", repo.updates[SettingKeyAvailabilityCheckEnabled])
}
