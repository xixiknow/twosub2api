...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) (shouldDisable bool) {
	...
	case 401:
		...
		// 前提：必须有 refresh_token，否则无法自动恢复，直接标记 error。
		if account.Type == AccountTypeOAuth && account.Platform != PlatformAntigravity && account.GetCredential("refresh_token") != "" {
			// 1. 失效缓存
			...
			// 非 OAuth / Antigravity OAuth：临时不可调度（替代永久禁用，允许自动恢复）
			msg := "Authentication failed (401): invalid or expired credentials"
			if upstreamMsg != "" {
			...
			until := time.Now().Add(time.Duration(cooldownMinutes) * time.Minute)
			if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, msg); err != nil {
				slog.Warn("apikey_401_set_temp_unschedulable_failed", "account_id", account.ID, "error", err)
			}
			shouldDisable = true
		}
	case 402:
