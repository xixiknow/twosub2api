...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// 支付设置
		adminSettings.GET("/payment", h.Admin.Setting.GetPaymentSettings)
		adminSettings.PUT("/payment", h.Admin.Setting.UpdatePaymentSettings)
		// Sora S3 存储配置
...
func registerPaymentOrderRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	orders := admin.Group("/payment-orders")
	{
		...
		orders.GET("/stats", h.Admin.PaymentOrder.Stats)
	}
}
