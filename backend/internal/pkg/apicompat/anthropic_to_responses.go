...
func convertAnthropicToolsToResponses(tools []AnthropicTool) []ResponsesTool {
	...
	for _, t := range tools {
		...
		out = append(out, ResponsesTool{
			...
			Parameters:  ensureSchemaProperties(t.InputSchema),
		})
	}
	return out
}

// ensureSchemaProperties 确保 JSON Schema 中包含 properties 字段。
// Codex 要求 function tool 的 parameters 必须有 properties，
// 但 Anthropic 的 input_schema 可能省略它（例如无参数的工具）。
func ensureSchemaProperties(schema json.RawMessage) json.RawMessage {
	if len(schema) == 0 {
