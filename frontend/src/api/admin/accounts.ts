 ...
 */
export async function fetchUpstreamModels(id: number): Promise<{ id: string; display_name?: string }[]> {
  const { data } = await apiClient.get<{ id: string; display_name?: string }[]>(`/admin/accounts/${id}/upstream-models`)
  return data
}
