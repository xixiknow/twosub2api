...
export async function getUserBreakdown(params: {
  start_date?: string
  end_date?: string
  endpoint?: string
  ...
  model?: string
  model_source?: string
}): Promise<{ users: import('@/types').UserBreakdownItem[] }> {
  const { data } = await apiClient.get('/admin/dashboard/user-breakdown', { params })
  return data
}
